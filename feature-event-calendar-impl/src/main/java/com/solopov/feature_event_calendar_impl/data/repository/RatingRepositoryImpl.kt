...
class RatingRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAllInstructorRatingsById(instructorId: String): List<Rating>? {
        return db.ratingDao().getAllInstructorRatingsById(instructorId)
            ?.map(ratingMappers::mapRatingLocalToRating)
    }

    override suspend fun getRatingByUserAndInstructorIds(
        userId: String,
        instructorId: String
    ): Rating? {
