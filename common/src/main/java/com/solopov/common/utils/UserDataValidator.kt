...
class UserDataValidator @Inject constructor(
    ...
    fun validatePassword(text: String): String? {
        ...
        if (!text.matches(
                resourceManager.getString(R.string.password_regex_one_uppercase_char).toRegex()
            )
        ) {
            return resourceManager.getString(R.string.must_contain_uppercase_character)
        }
        if (!text.matches(
                resourceManager.getString(R.string.password_regex_one_lowercase_char).toRegex()
            )
        ) {
            return resourceManager.getString(R.string.must_contain_lowercase_character)
        }
        if (!text.matches(
                resourceManager.getString(R.string.password_regex_one_special_char).toRegex()
            )
        ) {
